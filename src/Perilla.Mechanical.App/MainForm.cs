...
namespace Perilla.Mechanical.App
{
    ...
    public class MainForm : Form
    {
        ...
        private ToolStripButton _btnZoomFit, _btnZoomWidth, _btnZoom100;
        ...
        private Button _btnToggleLeft;
        private ListView _pageList;
        private ImageList _thumbImageList;
        private const int ThumbWidth = 120;
        private const int LeftPanelExpandedWidth = 200;
        private const int LeftPanelCollapsedWidth = 40;
        private bool _isLeftCollapsed = false;

        // 气泡大小预设按钮
        private ToolStripButton _btnSizeSmall, _btnSizeMedium, _btnSizeLarge;
        private ToolStripLabel _lblSizePreset;

        // 气泡颜色预设
        private struct BubbleColorPreset
        {
            public string Name;
        ...
        private BubbleColorPreset[] _bubbleColorPresets = new BubbleColorPreset[]
        {
            new BubbleColorPreset { Name = "红色", CircleColor = "#DC3C1E", FillColor = "#FFF0DC", TextColor = "#000000" },
            new BubbleColorPreset { Name = "蓝色", CircleColor = "#2563EB", FillColor = "#DBEAFE", TextColor = "#000000" },
            new BubbleColorPreset { Name = "绿色", CircleColor = "#16A34A", FillColor = "#DCFCE7", TextColor = "#000000" },
            new BubbleColorPreset { Name = "黄色", CircleColor = "#CA8A04", FillColor = "#FEF9C3", TextColor = "#000000" },
            new BubbleColorPreset { Name = "紫色", CircleColor = "#9333EA", FillColor = "#EDE9FE", TextColor = "#000000" },
            new BubbleColorPreset { Name = "橙色", CircleColor = "#EA580C", FillColor = "#FFEDD5", TextColor = "#000000" }
        };
        private int _currentColorPresetIndex = 0;
        private ComboBox _cmbBubbleColor;
        private Label _lblBubbleColor;

        // 缩放防抖
        private Timer _zoomDebounceTimer;
        private bool _zoomPending = false;
        private double _pendingZoomLevel = 1.0;
        private const int ZoomDebounceMs = 80;
        ...
        private Label _lblNum, _lblType, _lblText, _lblNote, _lblRadius;
        private TextBox _txtNum, _txtNote, _txtRadius;
        private ComboBox _cmbType;
        private TextBox _txtLinkedText;
        private Button _btnApplyProp;
        private Button _btnFeedbackCorrect, _btnFeedbackWrong;
        private Panel _feedbackPanel;
        private Label _feedbackLabel;

        // 训练样本存储
        private TrainingSampleStore _sampleStore;
        ...
        private PointD _dragBubbleOldCenter;

        // 气泡设置
        private double _defaultBubbleRadius = 12.0;
        private TrackBar _bubbleSizeSlider;
        private ToolStripControlHost _bubbleSizeSliderHost;
        private ToolStripLabel _bubbleSizeLabel;
        ...
        public MainForm()
        {
            try
            {
                ...
                SetupUndoRedo();
                InitZoomDebounce();
                UpdateSizePresetButtonState();

                try
                {
                    string samplePath = Path.Combine(
                        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                        "Perilla", "MechanicalPDF", "training_samples.json");
                    _sampleStore = new TrainingSampleStore(samplePath);
                }
                catch
                {
                    _sampleStore = null;
            ...
            catch (Exception ex)
            {
                MessageBox.Show("主窗体初始化失败：\r\n\r\n" + ex.Message + "\r\n\r\n" +
                    "堆栈：" + ex.StackTrace,
                    "启动错误", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                throw;
            }
        }
        ...
        private void BuildUi()
        {
            ...
            _btnDeleteBubble.Click += (s, e) => SafeInvoke(() => DeleteSelectedBubble());

            // 气泡大小预设按钮
            _lblSizePreset = new ToolStripLabel(" 大小:");
            _btnSizeSmall = new ToolStripButton { Text = "小", DisplayStyle = ToolStripItemDisplayStyle.Text, Padding = new Padding(6, 2, 6, 2) };
            _btnSizeSmall.Click += (s, e) => SetBubbleSizePreset(8);
            _btnSizeMedium = new ToolStripButton { Text = "中", DisplayStyle = ToolStripItemDisplayStyle.Text, Padding = new Padding(6, 2, 6, 2) };
            _btnSizeMedium.Click += (s, e) => SetBubbleSizePreset(16);
            _btnSizeLarge = new ToolStripButton { Text = "大", DisplayStyle = ToolStripItemDisplayStyle.Text, Padding = new Padding(6, 2, 6, 2) };
            _btnSizeLarge.Click += (s, e) => SetBubbleSizePreset(24);

            // 气泡大小滑块
            _bubbleSizeLabel = new ToolStripLabel(" 气泡:");
            _bubbleSizeSlider = new TrackBar
            {
                Minimum = 6,
                Maximum = 30,
                Value = 12,
            ...
            _bubbleSizeSlider.ValueChanged += (s, e) =>
            {
                _defaultBubbleRadius = _bubbleSizeSlider.Value;
                _bubbleSizeLabel.Text = " 气泡:" + _bubbleSizeSlider.Value + "pt";
                UpdateSizePresetButtonState();
            };
            _bubbleSizeSliderHost = new ToolStripControlHost(_bubbleSizeSlider);
            ...
            _btnZoomFit = new ToolStripButton { Text = "⊞ 适应窗口", DisplayStyle = ToolStripItemDisplayStyle.Text, Padding = new Padding(6, 2, 6, 2) };
            _btnZoomFit.Click += (s, e) => SafeInvoke(() => ZoomToFit());
            _btnZoomWidth = new ToolStripButton { Text = "↔ 适应宽度", DisplayStyle = ToolStripItemDisplayStyle.Text, Padding = new Padding(6, 2, 6, 2) };
            _btnZoomWidth.Click += (s, e) => SafeInvoke(() => ZoomToFitWidth());
            _btnZoom100 = new ToolStripButton { Text = "1:1", DisplayStyle = ToolStripItemDisplayStyle.Text, Padding = new Padding(6, 2, 6, 2) };
            _btnZoom100.Click += (s, e) => SafeInvoke(() => ApplyZoom(1.0));
            ...
            _toolbar.Items.AddRange(new ToolStripItem[] {
                _btnOpen, _sep1, _btnAuto, _btnAddBubble, _btnDeleteBubble,
                _lblSizePreset, _btnSizeSmall, _btnSizeMedium, _btnSizeLarge,
                _bubbleSizeLabel, _bubbleSizeSliderHost, _sep2,
                _btnUndo, _btnRedo, _sep3,
                _btnZoomFit, _btnZoomWidth, _btnZoom100, _sep4,
                _btnExportPdf, _btnExportPng, _btnExportExcel,
            ...
            _btnToggleLeft.FlatAppearance.BorderSize = 0;
            _btnToggleLeft.Click += OnToggleLeftPanel;
            _thumbImageList = new ImageList
            {
                ImageSize = new Size(ThumbWidth, 160),
            ...
            _pageList = new ListView
            {
                Dock = DockStyle.Fill,
                View = View.LargeIcon,
                LargeImageList = _thumbImageList,
                FullRowSelect = true,
                MultiSelect = false,
            ...
            _leftPanel.Controls.Add(_btnToggleLeft);
            ...
            SetDoubleBuffered(_picture);
            SetDoubleBuffered(_canvasContainer);
            ...
            _lblRadius = new Label { Text = "半径 (pt)：", Dock = DockStyle.Top, Height = 22, TextAlign = ContentAlignment.MiddleLeft };
            _txtRadius = new TextBox { Dock = DockStyle.Top, Height = 24 };
            _lblBubbleColor = new Label { Text = "气泡颜色：", Dock = DockStyle.Top, Height = 22, TextAlign = ContentAlignment.MiddleLeft };
            _cmbBubbleColor = new ComboBox { Dock = DockStyle.Top, Height = 24, DropDownStyle = ComboBoxStyle.DropDownList };
            foreach (var p in _bubbleColorPresets) _cmbBubbleColor.Items.Add(p.Name);
            _cmbBubbleColor.SelectedIndex = 0;
            ...
            _btnFeedbackCorrect = new Button
            {
                Text = "✓ 正确",
                Height = 28,
                Width = 60,
            ...
            _btnFeedbackCorrect.Click += OnFeedbackCorrect;
            _btnFeedbackWrong = new Button
            {
                Text = "✗ 错误",
                Height = 28,
                Width = 60,
            ...
            _btnFeedbackWrong.Click += OnFeedbackWrong;
            _feedbackPanel.Controls.Add(_btnFeedbackWrong);
            _feedbackPanel.Controls.Add(_btnFeedbackCorrect);
            _feedbackPanel.Controls.Add(_feedbackLabel);

            // 按倒序添加（Dock=Top 的顺序）
            _propertyPanel.Controls.Add(_btnApplyProp);
            _propertyPanel.Controls.Add(_feedbackPanel);
            ...
            _propertyPanel.Controls.Add(_cmbBubbleColor);
            _propertyPanel.Controls.Add(_lblBubbleColor);
            _propertyPanel.Controls.Add(_txtRadius);
            _propertyPanel.Controls.Add(_lblRadius);
        ...
        private void ApplyTheme(bool isDark)
        {
            ...
            _btnToggleLeft.BackColor = theme.TitleBarColor;
            _btnToggleLeft.ForeColor = theme.TitleTextColor;
            ...
            _feedbackPanel.BackColor = theme.PanelColor;
            _feedbackLabel.ForeColor = theme.TextColor;
            _propTitle.ForeColor = theme.TextColor;
            _lblNum.ForeColor = theme.TextColor;
            _lblRadius.ForeColor = theme.TextColor;
            _lblBubbleColor.ForeColor = theme.TextColor;
            ...
            _txtRadius.BackColor = isDark ? Color.FromArgb(65, 65, 70) : Color.White;
            _txtRadius.ForeColor = theme.TextColor;
            _cmbBubbleColor.BackColor = isDark ? Color.FromArgb(65, 65, 70) : Color.White;
            _cmbBubbleColor.ForeColor = theme.TextColor;
            ...
            _btnFeedbackCorrect.BackColor = theme.ButtonColor;
            _btnFeedbackCorrect.ForeColor = theme.TextColor;
            _btnFeedbackCorrect.FlatStyle = FlatStyle.Flat;
            _btnFeedbackCorrect.FlatAppearance.BorderColor = theme.BorderColor;
            _btnFeedbackWrong.BackColor = theme.ButtonColor;
            _btnFeedbackWrong.ForeColor = theme.TextColor;
            _btnFeedbackWrong.FlatStyle = FlatStyle.Flat;
            _btnFeedbackWrong.FlatAppearance.BorderColor = theme.BorderColor;
        ...
        private void UpdatePageList()
        {
            ...
            _thumbImageList.Images.Clear();
            if (_parser == null) return;

            for (int i = 0; i < _parser.PageCount; i++)
            {
                var size = _parser.GetPageSize(i);
                double scale = (double)ThumbWidth / size.Width;
                int thumbHeight = Math.Max(1, (int)(size.Height * scale));

                // 使用低缩放比例渲染缩略图
                Bitmap thumbBmp = null;
                try
                {
                    thumbBmp = _parser.RenderPageToBitmap(i, Math.Max(0.3, scale));
                    // 缩放到目标宽度
                    var scaled = new Bitmap(ThumbWidth, thumbHeight);
                    using (var g = Graphics.FromImage(scaled))
                    {
                        g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBicubic;
                        g.Clear(Color.White);
                        g.DrawImage(thumbBmp, 0, 0, ThumbWidth, thumbHeight);
                    }
                    thumbBmp.Dispose();
                    _thumbImageList.Images.Add(scaled);
                }
                catch
                {
                    // 缩略图渲染失败，使用占位图
                    var placeholder = new Bitmap(ThumbWidth, thumbHeight);
                    using (var g = Graphics.FromImage(placeholder))
                    {
                        g.Clear(Color.LightGray);
                        g.DrawString("第 " + (i + 1) + " 页", Font, Brushes.Gray, 10, thumbHeight / 2);
                    }
                    _thumbImageList.Images.Add(placeholder);
                }

                var item = new ListViewItem("第 " + (i + 1) + " 页", i) { Tag = i };
        ...
        private void OnRunAutomation(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                opts.Numbering.BubbleRadius = _defaultBubbleRadius;
        ...
        {
            if (_parser == null || _picture.Image == null) return;

            // 1) 添加气泡模式：单击放置新气泡
            if (_pendingAddBubble && e.Button == MouseButtons.Left)
            {
                ...
                var preset = _bubbleColorPresets[_currentColorPresetIndex];
                int nextNum = GetNextGlobalBubbleNumber();
                var bubble = new Bubble
                {
                    Number = nextNum,
                    Label = nextNum.ToString(),
                    Center = new PointD(pdfPt.X, pdfPt.Y),
                    Radius = _defaultBubbleRadius,
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (clicked != null)
                {
                    _selectedBubble = clicked;
                    _isDraggingBubble = true;
                    _dragBubbleOldCenter = new PointD(clicked.Center.X, clicked.Center.Y);
                    SelectBubbleInList(clicked);
                ...
                else
                {
                    ...
                    _panStartScroll = new Point(
                        _canvasContainer.AutoScrollPosition.X,
                        _canvasContainer.AutoScrollPosition.Y);
        ...
        private void OnPictureMouseMove(object sender, MouseEventArgs e)
        {
            ...
            // 气泡拖拽中
            if (_isDraggingBubble && _selectedBubble != null)
            {
                var size = _parser.GetPageSize(_currentPage);
                PointF pdfPt = PixelToPdfPoint(new PointF(e.X, e.Y), size);
                _selectedBubble.Center = new PointD(pdfPt.X, pdfPt.Y);
                RedrawPicture();
                return;
            }
            ...
            if (_isPanning)
            {
                ...
                _canvasContainer.AutoScrollPosition = new Point(
        ...
        private void OnPictureMouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (_isDraggingBubble && _selectedBubble != null)
            {
                ...
                var dx = _selectedBubble.Center.X - _dragBubbleOldCenter.X;
                var dy = _selectedBubble.Center.Y - _dragBubbleOldCenter.Y;
                if (Math.Abs(dx) > 0.01 || Math.Abs(dy) > 0.01)
                {
                    _undoRedo.Execute(new MoveBubbleCommand(
                        _selectedBubble,
                        _dragBubbleOldCenter,
                        new PointD(_selectedBubble.Center.X, _selectedBubble.Center.Y)));
                    UpdateStatus("已移动气泡 #" + _selectedBubble.Label);
                }
                _picture.Cursor = Cursors.Default;
                return;
            }
        ...
        private void OnPictureMouseWheel(object sender, MouseEventArgs e)
        {
            ...
            double newZoom = (_zoomPending ? _pendingZoomLevel : _zoomLevel) * factor;
        ...
        private void DrawBubblesOnBitmap(Bitmap bmp, List<Bubble> bubbles, SizeF pageSize, double scale)
        {
            using (var g = Graphics.FromImage(bmp))
            {
                ...
                foreach (var b in bubbles)
                {
                    ...
                    var circleColor = GetBubbleCircleColor(b);
                    var fillColor = GetBubbleFillColor(b);
                    var textColor = GetBubbleTextColor(b);
                    ...
                    if (b.SourceBounds.Width > 0 && b.SourceBounds.Height > 0)
                    {
                        ...
                        using (var leaderPen = new Pen(circleColor, Math.Max(1.0f, r * 0.12f)))
                        {
        ...
        private void LoadBubbleToPropertyPanel(Bubble b)
        {
            ...
            _txtRadius.Text = b.Radius.ToString("0.##");
            _cmbType.SelectedIndex = (int)b.Kind;
            _txtLinkedText.Text = b.LinkedText ?? "";
            _txtNote.Text = b.IsManual ? "手动标注" : "AI 识别";

            int colorIdx = 0;
            for (int i = 0; i < _bubbleColorPresets.Length; i++)
            {
                if (_bubbleColorPresets[i].CircleColor == b.CircleColor) { colorIdx = i; break; }
            }
            _cmbBubbleColor.SelectedIndex = colorIdx;
        }
        ...
        private void OnApplyPropertyChanges(object sender, EventArgs e)
        {
            ...
            double r;
            if (double.TryParse(_txtRadius.Text, out r) && r >= 3 && r <= 100)
            {
                _selectedBubble.Radius = r;
            }
            if (_cmbType.SelectedIndex >= 0)
            {
                _selectedBubble.Kind = (RecognitionKind)_cmbType.SelectedIndex;
            }
            if (_cmbBubbleColor.SelectedIndex >= 0)
            {
                var preset = _bubbleColorPresets[_cmbBubbleColor.SelectedIndex];
                _selectedBubble.CircleColor = preset.CircleColor;
                _selectedBubble.FillColor = preset.FillColor;
                _selectedBubble.TextColor = preset.TextColor;
                _currentColorPresetIndex = _cmbBubbleColor.SelectedIndex;
            }
            RenumberBubbles();
            RedrawPicture();
            UpdateBubbleList();
        ...
        private void OnFeedbackCorrect(object sender, EventArgs e)
        {
            if (_selectedBubble == null || _sampleStore == null) return;
            var sample = new TrainingSample
            {
                PdfFileName = _currentPdf != null ? Path.GetFileName(_currentPdf) : "",
                PageIndex = _selectedBubble.PageIndex,
                ElementRegion = _selectedBubble.SourceBounds,
                CorrectType = _selectedBubble.Kind,
                RecognizedType = _selectedBubble.IsManual ? (RecognitionKind?)null : _selectedBubble.Kind,
                RawText = _selectedBubble.LinkedText ?? "",
                IsCorrect = true,
                Source = _selectedBubble.IsManual ? "手动标注" : "AI识别"
            };
            _sampleStore.AddSample(sample);
            UpdateStatus("已记录正确样本（累计 " + _sampleStore.Count + " 个）");
        }

        private void OnFeedbackWrong(object sender, EventArgs e)
        {
            if (_selectedBubble == null || _sampleStore == null) return;
            var sample = new TrainingSample
            {
            ...
            _sampleStore.AddSample(sample);
            UpdateStatus("已记录错误样本（累计 " + _sampleStore.Count + " 个）");
        }
        ...
        private void OnFormKeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.Control)
                {
                    ...
                    else if (e.KeyCode == Keys.D0 || e.KeyCode == Keys.NumPad0) { ZoomToFit(); e.Handled = true; }
                    else if (e.KeyCode == Keys.D1 || e.KeyCode == Keys.NumPad1) { ApplyZoom(1.0); e.Handled = true; }
                    else if (e.KeyCode == Keys.W) { ZoomToFitWidth(); e.Handled = true; }
                    else if (e.KeyCode == Keys.Z) { SafeInvoke(() => { _undoRedo?.Undo(); RedrawPicture(); UpdateBubbleList(); }); e.Handled = true; }
                    else if (e.KeyCode == Keys.Y) { SafeInvoke(() => { _undoRedo?.Redo(); RedrawPicture(); UpdateBubbleList(); }); e.Handled = true; }
                }
        ...
        private void ZoomToFit()
        {
            if (_parser == null) return;
            var size = _parser.GetPageSize(_currentPage);
            int viewW = _canvasContainer.ClientSize.Width - _canvasContainer.Padding.Horizontal - 4;
            int viewH = _canvasContainer.ClientSize.Height - _canvasContainer.Padding.Vertical - 4;
            if (viewW <= 0 || viewH <= 0) return;
            double effectiveScale = _renderScale;
            double imgW = size.Width * effectiveScale;
            double imgH = size.Height * effectiveScale;
            double zoom = Math.Min(viewW / imgW, viewH / imgH);
            ApplyZoom(zoom);
        }
        ...
        // 左侧面板折叠/展开
        // ============================================================
        private void OnToggleLeftPanel(object sender, EventArgs e)
        {
            _isLeftCollapsed = !_isLeftCollapsed;
            if (_isLeftCollapsed)
            {
                _leftPanel.Width = LeftPanelCollapsedWidth;
                _leftTitle.Visible = false;
                _pageList.Visible = false;
                _btnToggleLeft.Text = "▶";
            }
            else
            {
                _leftPanel.Width = LeftPanelExpandedWidth;
                _leftTitle.Visible = true;
                _pageList.Visible = true;
                _btnToggleLeft.Text = "◀";
            }
        }
        ...
        // 气泡大小预设
        // ============================================================
        private void SetBubbleSizePreset(double size)
        {
            _defaultBubbleRadius = size;
            _bubbleSizeSlider.Value = (int)Math.Max(_bubbleSizeSlider.Minimum,
                Math.Min(_bubbleSizeSlider.Maximum, size));
            UpdateSizePresetButtonState();
        }

        private void UpdateSizePresetButtonState()
        {
            double r = _defaultBubbleRadius;
            _btnSizeSmall.Font = new Font(_btnSizeSmall.Font, Math.Abs(r - 8) < 0.5 ? FontStyle.Bold : FontStyle.Regular);
            _btnSizeMedium.Font = new Font(_btnSizeMedium.Font, Math.Abs(r - 16) < 0.5 ? FontStyle.Bold : FontStyle.Regular);
            _btnSizeLarge.Font = new Font(_btnSizeLarge.Font, Math.Abs(r - 24) < 0.5 ? FontStyle.Bold : FontStyle.Regular);
        }
        ...
        // 颜色辅助：获取气泡实际颜色
        // ============================================================
        private Color GetBubbleCircleColor(Bubble b)
        {
            if (b == _selectedBubble) return Color.FromArgb(255, 120, 0);
            return HtmlToColor(b.CircleColor, Color.FromArgb(220, 60, 30));
        }

        private Color GetBubbleFillColor(Bubble b)
        {
            if (b == _selectedBubble) return Color.FromArgb(255, 220, 180);
            return HtmlToColor(b.FillColor, Color.FromArgb(255, 240, 220));
        }

        private Color GetBubbleTextColor(Bubble b)
        {
            return HtmlToColor(b.TextColor, Color.Black);
        }

        // ============================================================
        // 全局连续编号 - 获取下一个编号
        // ============================================================
        private int GetNextGlobalBubbleNumber()
        {
            int max = 0;
            foreach (var pageBubbles in _pageBubbles)
            {
                foreach (var b in pageBubbles)
                {
                    if (b.Number > max) max = b.Number;
                }
            }
            return max + 1;
        }
        ...
        {
            int n = 1;
            for (int pi = 0; pi < _pageBubbles.Count; pi++)
            {
                var pageBubbles = _pageBubbles[pi];
        ...
        // 缩放防抖
        // ============================================================
        private void InitZoomDebounce()
        {
            _zoomDebounceTimer = new Timer { Interval = ZoomDebounceMs };
        ...
        private void DebouncedZoom(double newZoom)
        {
            ...
            _lblZoom.Text = "缩放：" + newZoom.ToString("P0");
            _statusZoom.Text = "缩放：" + newZoom.ToString("P0");
        }
