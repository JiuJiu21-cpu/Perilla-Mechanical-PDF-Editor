    ...
    public class MainForm : Form
    {
        ...
        private void OnRunAutomation(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // 合并到 _pageBubbles（自动气泡替换旧自动气泡；保留手动）
                for (int i = 0; i < results.Count; i++)
                {
                    var pageBubbles = _pageBubbles[i];
                    pageBubbles.RemoveAll(b => !b.IsManual);
                    foreach (var b in results[i].Bubbles)
                    {
                        if (!b.IsManual) pageBubbles.Add(b);
                    }
                }
        ...
        private void RedrawPicture()
        {
            ...
            {
                var size = _parser.GetPageSize(_currentPage);
                var bmp = _parser.RenderPageToBitmap(_currentPage, _renderScale);
                if (bmp == null) return;
                var bubbles = _pageBubbles[_currentPage];
                var exporter = new ImageExporter();
                exporter.DrawBubblesOnBitmap(bmp, bubbles, size, _renderScale);
