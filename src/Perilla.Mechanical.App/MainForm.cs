...
namespace Perilla.Mechanical.App
{
    ...
    public class MainForm : Form
    {
        ...
        private void OnPictureMouseWheel(object sender, MouseEventArgs e)
        {
            ...
            // 记录缩放前鼠标在PDF中的坐标
            PointF pdfBefore = PixelToPdfPoint(new PointF(e.X, e.Y), size);
            ...
            // 调整滚动位置，使鼠标指向的PDF点保持在鼠标下方
            PointF pdfAfter = PixelToPdfPoint(new PointF(e.X, e.Y), size);
            double effectiveScale = _renderScale * _zoomLevel;
            int scrollDx = (int)((pdfAfter.X - pdfBefore.X) * effectiveScale);
            int scrollDy = (int)((pdfBefore.Y - pdfAfter.Y) * effectiveScale);
            _canvasContainer.AutoScrollPosition = new Point(
                _canvasContainer.AutoScrollPosition.X + scrollDx,
                _canvasContainer.AutoScrollPosition.Y + scrollDy);
